-- ─────────────────────────────────────────────────────────────────────────────
-- Add Meta ad-preview embeds so video ads actually render (and play) on the
-- dashboard. Run in the Supabase SQL editor, AFTER the earlier 20260523_*
-- migrations. Idempotent.
--
-- Meta no longer exposes a raw video file URL for ad videos, so the <video>
-- approach can't work. Instead the sync now stores `preview_url` — the src of
-- Meta's official ad-preview iframe, which renders the real ad with the video
-- playable inline. This rebuilds get_meta_ad_creatives() to return it.
-- ─────────────────────────────────────────────────────────────────────────────
...
-- Sanity check — preview_url should be populated after the next sync.
SELECT studio_slug, ad_name, media_type, (preview_url IS NOT NULL) AS has_preview
